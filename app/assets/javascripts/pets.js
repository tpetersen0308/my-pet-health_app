  ...
  $("#js-search").submit(function(event){
    ...
    posting.success(function(data){
      ...
      if(data){
        ...
        newPetHTML += `<h4>${newPet.name}</h4><ul><li>Type: ${newPet.sex} ${newPet.species}</li><li><a href='/users/${newOwner.id}'>${newOwner.displayFullName()}</a></li><li>Age: ${newPet.age}</li>${vetsHTML}</ul>`;
      } else {
